...
| inputModePanel              | Dictionary | 入力モードのモーダルの色設定            |
| skkservAutoDisableThreshold | Number     | skkservの接続エラーの連続回数閾値       |
| showMarkedTextMarker        | String     | ▽と▼の表示                              |
| ignoreLeadingSpacesWhenRegistering | Boolean | 単語登録中に先頭のスペースを無視するか |
| backToSelectingFromRegistering | Boolean | 単語登録中に空文字列で前候補キーを押したときに候補選択に戻るか |
| backToSelectingFromRegisteringByBackspace | Boolean | 単語登録中に空文字列でバックスペースキーを押したときに候補選択に戻るか |
...
`#!use-default` という行を書くことで、デフォルトのかなルールを読み込んだ上で差分だけを定義することが可能です。

ファイルを配置したのに反映されない場合は設定画面のログでエラーが出てないか確認してみてください。
